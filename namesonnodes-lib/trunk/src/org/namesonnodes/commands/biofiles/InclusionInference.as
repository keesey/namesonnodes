package org.namesonnodes.commands.biofiles
{
	...
	public final class InclusionInference
	{
			...
			phylogeny:DirectedGraph):Boolean
		{
			...
			if (weight > 0.0001)
			{
				const directPredecessors:FiniteSet = phylogeny.directPredecessors(taxon);
		...
		private static function derivePhylogeny(heredities:Object):HashNetwork
		{
			...
			for each (var heredity:Heredity in heredities)
				if (heredity.weight == 0.0)
					...
					phylogeny.createWeightedEdge(heredity.predecessor, heredity.successor, heredity.weight);
			return phylogeny;
		}
		private static function findChildWeight(taxon:TaxonIdentifier, childWeights:Dictionary,
			phylogeny:DirectedGraph):Number
		{
			const existingWeight:* = childWeights[taxon.entity];
			if (existingWeight is Number)
				return existingWeight;
			const children:FiniteSet = phylogeny.directSuccessors(taxon);
			if (children.empty)
			{
				childWeights[taxon.entity] = 0.0;
				return 0.0;
			}
			...
			childWeights[taxon.entity] = average;
			return average;
		}
		public static function inferInclusions(scoreMap:Dictionary, heredities:Object):FiniteSet
		{
			// Derive phylogeny.
			...
			for (state in normalizedInferences)
				for each (var inferredSubset:TaxonIdentifier in normalizedInferences[state])
				{
		...
		public static function inferSet(baseSet:FiniteSet, phylogeny:DirectedGraph):FiniteSet
		{
			...
			for each (taxon in phylogeny.vertices)
